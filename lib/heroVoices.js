...
const ANIME_VOICES = {
  ...
  loli: {
    ...
    engineType: 'loli',
    intro: 'わくわく！',
    description: 'High-pitched, ultra-cute chibi / loli anime girl voice (Waku waku!)',
 ...
 * @returns {Promise<Buffer>}
 */
async function generateVoicevoxTTSWithRetry(speaker = 12, text, isLoli = false) {
  let lastErr = null;
  const backoffs = [0, 600]; // 2 fast attempts
  ...
  // Graceful fallback to Edge Neural (guarantees voice notes never fail or 429)
  console.warn(`[Voicevox] Cloud cluster busy, using instant Edge Neural fallback (${lastErr?.message})`);
  try {
    if (isLoli || speaker === 3 || speaker === 4 || speaker === 8 || speaker === 14) {
      // Ultra-cute anime girl / chibi voice
      return await synthesizeEdgeNeuralChunk(text, 'ja-JP-NanamiNeural', 'ja-JP', '+5%', '+25Hz');
    } else {
      // Male / cool anime character voice
      return await synthesizeEdgeNeuralChunk(text, 'ja-JP-KeitaNeural', 'ja-JP', '+0%', '-10Hz');
    }
  } catch (fallbackErr) {
 ...
 * Supports Pakistani Urdu (Sara, Gul, Asad) and authentic Anime characters (Voicevox + Edge fallback).
...
async function generateHeroTTS(voiceKey, rawText) {
  ...
  if (character.id === 'sara' || character.id === 'gul' || character.id === 'asad' || character.engineType === 'urdu' || character.engineType === 'sara' || character.engineType === 'asad') {
    ...
    // 2. Otherwise, use authentic Voicevox Studio Anime Engine with fallback
    ...
    const isLoli = character.id === 'loli' || character.engineType === 'loli';
    const chunks = splitTextIntoChunks(speechText, 250);

    if (chunks.length <= 1) {
      finalBuffer = await generateVoicevoxTTSWithRetry(speakerId, speechText, isLoli);
    } else {
      const parts = [];
      for (const part of chunks) {
        const buf = await generateVoicevoxTTSWithRetry(speakerId, part, isLoli);
        parts.push(buf);
