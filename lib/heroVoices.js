...
const ANIME_VOICES = {
  sara: {
    id: 'sara',
    name: 'Sara',
    title: 'Pakistani Urdu Girl',
    anime: 'Pakistani Urdu Voice',
    emoji: '🧕',
    isSpecialEngine: true,
    engineType: 'sara',
    intro: '',
    description: 'Sweet, natural human girl voice with authentic Pakistani Urdu pronunciation',
...
/**
 * Transliterate Roman Urdu to Urdu Nastaliq script via Google Input Tools
 * Allows natural native Urdu pronunciation when text is written in English alphabet
 */
async function transliterateRomanUrdu(text) {
  try {
    const url = `https://inputtools.google.com/request?text=${encodeURIComponent(text)}&itc=ur-t-i0-und&num=1`;
    const res = await axios.get(url, { timeout: 3500 });
    if (res.data && res.data[0] === 'SUCCESS' && res.data[1]?.[0]?.[1]?.[0]) {
...
/**
 * Synthesize speech using Microsoft Edge Neural Voice for Pakistani Urdu (ur-PK-UzmaNeural)
 * Produces natural human female voice with native Pakistani Urdu accent and inflection.
 * Zero CPU/RAM load on host container.
 */
function synthesizeEdgeNeuralUrdu(text) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(new Error('Edge Neural TTS connection timed out.'));
    }, 12000);
      ...
      if (isBinary) {
        ...
        if (textMsg.includes('Path:turn.end')) {
          ...
          if (finalAudio.length > 500) {
            ...
            reject(new Error('Empty audio buffer received from Edge TTS'));
          }
        }
      }
    });
...
/**
 * Authentic Pakistani Urdu Human Girl Voice for Sara
 * Powered by Microsoft Neural Voice (ur-PK-UzmaNeural)
 * Native Pakistani Urdu pronunciation, natural human girl tone, zero robotic artifacts.
 * Automatically transliterates Roman Urdu to Urdu script for perfect pronunciation.
...
async function generateSaraTTS(rawText) {
  ...
  // Check if text is in Urdu script. If Roman Urdu, transliterate to authentic Urdu script
  const isUrduScript = /[\u0600-\u06FF]/.test(clean);
  const speechText = isUrduScript ? clean : await transliterateRomanUrdu(clean);

  // 1. Primary Engine: Microsoft Edge Neural Voice (ur-PK-UzmaNeural) - Natural human Pakistani girl
  try {
    return await synthesizeEdgeNeuralUrdu(speechText);
  } catch (err) {
    console.warn(`[Sara Edge TTS] Neural engine failed (${err.message}), trying backup Urdu engine...`);
  }

  // 2. Fallback Engine: Google Translate Neural Urdu endpoint
  try {
  ...
  throw new Error('Failed to synthesize Sara Pakistani Urdu voice. Please try again.');
}
...
function getHeroCatalog() {
  const lines = [
    ...
    '',
    '🌸 *PAKISTANI URDU VOICE (HUMAN GIRL):*',
