...
async function generateHeroTTS(voiceKey, rawText, options = {}) {
  ...
  if (character.id === 'aizen') {
    // Sosuke Aizen Mastermind Engine (Fast ~0.5s MP3 by default + Voicevox HD option)
    const aizenLang = /[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff]/.test(cleanText) ? 'ja' : 'en-uk';
    if (isHd) {
      try {
        finalBuffer = await generateVoicevoxTTS(character.speaker || 13, cleanText);
        usedEngine = 'voicevox_villain_hd';
      } catch (vvErr) {
        console.warn(`[TTS Engine] Voicevox HD failed (${vvErr.message}), falling back to fast Aizen engine...`);
        finalBuffer = await generateFastAnimeTTS(aizenLang, cleanText);
        usedEngine = 'fast_aizen_fallback';
      }
    } else {
      try {
        finalBuffer = await generateFastAnimeTTS(aizenLang, cleanText);
        usedEngine = 'fast_aizen_mastermind';
      } catch (fastErr) {
        console.warn(`[TTS Engine] Fast Aizen failed (${fastErr.message}), falling back to studio villain...`);
        finalBuffer = await generateVoicevoxTTS(character.speaker || 13, cleanText);
