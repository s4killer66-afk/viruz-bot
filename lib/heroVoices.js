...
// In-memory cache for fast, zero-delay responses (up to 50 items)
...
async function generateVoicevoxTTS(speaker = 47, text) {
  ...
  // Poll status up to 10 times (each 500ms -> max 5s wait)
  for (let i = 0; i < 10; i++) {
...
/**
 * Fallback synthesizer using Google TTS if Voicevox is slow or unavailable
 */
async function generateGoogleFallbackTTS(lang, text) {
 ...
 * Dual-engine: Real Anime Voicevox Engine primary + Lightweight Fallback secondary.
...
async function generateHeroTTS(voiceKey, rawText) {
  ...
  try {
    ...
    console.warn(`[TTS Engine] Voicevox synthesis failed (${vvErr.message}), falling back to secondary engine...`);
    usedEngine = 'google_fallback';
    finalBuffer = await generateGoogleFallbackTTS(character.lang || 'ja', cleanText);
  }
