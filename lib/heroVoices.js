...
const ENGLISH_WORDS = new Set([
  ...
  'welcome','everyone','please','thank','thanks','you','group','hello','hi','hey',
  'good','morning','evening','night','friend','friends','rules','check','admin','owner',
  'bot','game','play','join','leave','member','members','people','text','call','send',
  'link','video','audio','image','photo','status','brother','sister','nice','great','happy',
  'today','tomorrow','yesterday','love','gaming','player','win','victory','congratulations'
]);

const ROMAN_URDU_WORDS = new Set([
  'kya','hai','hain','ho','me','mein','ka','ki','ke','ko','se','par','bhai','kese','kaise',
  'ap','aap','tum','mera','meri','mere','tera','teri','tere','kahan','kaha','nahi','nahin','na',
  'ye','yeh','wo','woh','karo','karna','karni','karne','raha','rahi','rahe','theek','shukriya',
  'suno','bol','bolti','bolta','baat','acha','achi','achha','achhi','bohot','bahut','bhi','toh',
  'aur','ab','sab','log','dost','dosto','zara','dekh','lo','lekin','magar','kyun','kyu','kaisa',
  'kaisi','chalo','chal','gaya','gayi','gaye','aao','jao','kuch','koi','hum','humein','tumhein',
  'unko','isko','unhe','ishe','wahan','yahan','idhar','udhar','phir','kyon','kisko','sun'
]);

/**
 * Transliterate Roman Urdu text into authentic Urdu script via Google Input Tools
 * @param {string} text
 * @returns {Promise<string>}
 */
async function transliterateUrdu(text) {
  try {
    ...
    if (res.data && res.data[0] === 'SUCCESS' && Array.isArray(res.data[1])) {
      return res.data[1].map(item => item[1]?.[0] || item[0]).join(' ');
    }
  } catch (err) {
    console.warn(`[Sara Transliterate] Input tools unavailable (${err.message}), falling back to direct dictionary mapping`);
  }
  return text;
}
 ...
 * - Pure Urdu script (nastaliq): Preserved 100% as-is.
 * - Pure English: Preserved 100% as English (spoken fluently with native English phonetics).
 * - Roman Urdu or mixed: Fully transliterated into authentic Urdu script so Edge Neural
 *   (ur-PK-UzmaNeural) speaks every single word without skipping words or dropping prefixes/suffixes.
 * @param {string} input
 * @returns {Promise<string>}
 */
async function prepareTextForSara(input) {
  const clean = (input || '').trim();
  if (!clean) return '';

  // 1. If text has no Latin letters (pure Urdu script, emojis, punctuation), return directly
  if (!/[a-zA-Z]/.test(clean)) {
    return clean;
  }

  // 2. Score English vs Roman Urdu words
  const words = clean.toLowerCase().match(/[a-z]+/g) || [];
  let engScore = 0;
  let urduScore = 0;
  for (const w of words) {
    if (ENGLISH_WORDS.has(w)) engScore++;
    if (ROMAN_URDU_WORDS.has(w)) urduScore++;
  }

  // 3. Pure English sentence (contains English words and zero Roman Urdu markers): keep pure Latin text
  if (engScore > 0 && urduScore === 0) {
    return clean;
  }

  // 4. Roman Urdu or mixed: Transliterate to authentic Urdu script so ur-PK-UzmaNeural reads 100% of words
  let transliterated = await transliterateUrdu(clean);

  // If transliteration returned original Latin (e.g. offline fallback), apply fallback dictionary
  if (transliterated === clean) {
    const tokens = clean.split(/([a-zA-Z0-9_]+|[^\s\w]+|\s+)/).filter(Boolean);
    return tokens.map(token => {
      const lower = token.toLowerCase();
      return ROMAN_URDU_DICTIONARY[lower] || token;
    }).join('');
  }

  // 5. Post-process known transliteration adjustments for natural pronunciation
  const tokens = transliterated.split(' ');
  const hasBhai = /\bbhai\b/i.test(clean);
  const hasSara = /\bsara\b/i.test(clean);

  const adjusted = tokens.map(t => {
    if (hasBhai && (t === 'بھی' || t === 'بهی')) return 'بھائی';
    if (hasSara && (t === 'سارا' || t === 'سارہ')) return 'سارہ';
    return t;
  });
...
async function generateSaraTTS(rawText) {
  ...
  const processed = await prepareTextForSara(safeText);
