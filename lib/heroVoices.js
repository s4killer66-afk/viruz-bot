...
/**
 * Synthesize speech using Microsoft Edge Neural Voice for Pakistani Urdu & English (ur-PK-UzmaNeural)
 * Produces natural human female voice with native Pakistani pronunciation and authentic accent.
 * Correctly pronounces English words and Urdu with smooth Pakistani intonation.
...
/**
 * Authentic Pakistani Urdu & English Human Girl Voice for Sara
 * Powered by Microsoft Neural Voice (ur-PK-UzmaNeural)
 * Flawlessly pronounces Urdu and English words with natural human girl Pakistani accent.
 * Zero robotic monotone, zero heavy server load.
...
async function generateSaraTTS(rawText) {
  ...
  // 1. Primary Engine: Microsoft Edge Neural Voice (ur-PK-UzmaNeural)
  // Perfectly handles Urdu script, Roman Urdu, and English words with native Pakistani accent
  try {
    return await synthesizeEdgeNeuralUrdu(clean);
  } catch (err) {
    console.warn(`[Sara Edge TTS] Neural engine failed (${err.message}), trying backup Urdu/English engine...`);
  }

  // 2. Fallback Engine: Google Translate Neural endpoint
  try {
    const isUrduScript = /[\u0600-\u06FF]/.test(clean);
    const targetLang = isUrduScript ? 'ur' : 'en-IN';
  ...
  throw new Error('Failed to synthesize Sara Pakistani voice. Please try again.');
}
