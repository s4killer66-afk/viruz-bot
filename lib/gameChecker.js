...
const MLBB_TIMELINE_ANCHORS = [
  ...
  { time: new Date('2021-07-01').getTime(), userId: 900000000, zoneId: 11500 },
  { time: new Date('2022-01-01').getTime(), userId: 1080000000, zoneId: 13200 },
  { time: new Date('2022-07-01').getTime(), userId: 1250000000, zoneId: 15500 },
  { time: new Date('2023-01-01').getTime(), userId: 1420000000, zoneId: 18000 },
  { time: new Date('2023-07-01').getTime(), userId: 1600000000, zoneId: 21000 },
  { time: new Date('2024-01-01').getTime(), userId: 1760000000, zoneId: 24200 },
  { time: new Date('2024-07-01').getTime(), userId: 1900000000, zoneId: 27500 },
  { time: new Date('2025-01-01').getTime(), userId: 2020000000, zoneId: 31000 },
  { time: new Date('2025-07-01').getTime(), userId: 2120000000, zoneId: 34000 },
  { time: new Date('2026-01-01').getTime(), userId: 2180000000, zoneId: 36500 },
...
function getWIBTimestamp() {
  const d = new Date();
  const wibTime = new Date(d.getTime() + (7 * 60 + d.getTimezoneOffset()) * 60000);
  const pad = n => String(n).padStart(2, '0');
...
function getMLRankAndLevel(id, zone) {
  ...
  const rankIdx = getDeterministicState(id, 'rank_seed_28') % RANKS.length;
  const currentRank = RANKS[rankIdx];
  const highestIdx = Math.min(RANKS.length - 1, rankIdx + (getDeterministicState(id, 'highest_rank') % 4));
  const highestRank = RANKS[highestIdx];

  let level;
  if (z >= 35000) {
    level = 10 + (getDeterministicState(id, 'level_seed_44') % 20);
  } else if (z >= 20000) {
    level = 25 + (getDeterministicState(id, 'level_seed_44') % 35);
  } else {
    level = 45 + (getDeterministicState(id, 'level_seed_44') % 60);
  }

  const season = z >= 35000 && level < 20 ? 0 : (30 + (getDeterministicState(id, 'season_seed') % 5));
...
async function checkMobileLegends(rawId, zoneId) {
  ...
  const lastLogin = getWIBTimestamp();
