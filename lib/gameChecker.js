...
function getAccountCreationEstimate(zone) {
  const z = parseInt(zone, 10);
  if (isNaN(z) || z < 1000) {
    return '~2020-01-15';
  }
  const startDate = new Date('2016-07-15').getTime();
  const msPerZone = 8496317.28;
  const estTime = Math.min(Date.now() + 30 * 86400000, startDate + Math.max(0, z - 1000) * msPerZone);
...
async function checkMobileLegends(rawId, zoneId) {
  ...
  const createdDate = getAccountCreationEstimate(zone);
