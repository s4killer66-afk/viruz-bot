...
class SafetyManager {
  constructor() {
    ...
    // Safety thresholds (tuned for zero WhatsApp ban risk & responsive bot)
    this.MIN_PACING_DELAY_MS = 400;       // Minimum delay between consecutive socket sends
    this.MAX_PACING_DELAY_MS = 750;       // Maximum random human jitter
    this.MAX_SENDS_PER_GROUP_PER_MIN = 12; // Max messages per group per minute
    this.MAX_SENDS_PER_DM_PER_MIN = 6;     // Max messages to non-owner DM per minute
    this.MAX_GLOBAL_SENDS_PER_MIN = 30;    // Max total outbound messages across all chats in 60s
    this.MASS_MENTION_COOLDOWN_SEC = 30;   // Cooldown between .tagall/.hidetag per group
    this.KICK_COOLDOWN_MS = 1500;          // Safe delay between kicks (prevents mass-kick flag)
  }
  ...
  async safeSend(sock, jid, content, options = {}) {
    if (!this.canSend(jid)) {
      console.log(`[AntiBan] Rate limited: skipping or pacing send to ${jid.substring(0, 15)}...`);
      return null;
    }
