...
class GroupModerator {
  ...
  isGroupAdmin(jid, groupMetadata) {
    ...
    const userPhone = cleanUser.split('@')[0];
    if (config.ownerNumbers && config.ownerNumbers.some(num => cleanUser.includes(num) || num.includes(userPhone))) {
      return true;
    }

    const participant = groupMetadata.participants.find(p => this.cleanJid(p.id) === cleanUser);
    return participant ? (participant.admin === 'admin' || participant.admin === 'superadmin') : false;
  }

  /**
   * Handle incoming sticker in a group
   * Rule: 3rd sticker spam = Warning, 4th = Auto-kick
   */
   ...
   * Rule: 4th repeat/spam = Warning, 5th = Auto-kick
   */
  async handleMessageSpam(sock, groupId, senderJid, text, groupMetadata) {
    ...
    // 5th repeated message: AUTO KICK
