...
// Cache up to 24 hours of messages (86400 seconds) with up to 10,000 keys
const messageCache = new NodeCache({ stdTTL: 86400, checkperiod: 600, maxKeys: 10000 });
...
class AntiDeleteManager {
  ...
  storeMessage(msg) {
    ...
    // Also store by composite key if remoteJid exists
    if (msg.key.remoteJid) {
      messageCache.set(`${msg.key.remoteJid}_${id}`, msg);
    }
  }
  ...
  async handleRevoke(sock, msg) {
    ...
    let originalMsg = messageCache.get(revokedId);
    if (!originalMsg && remoteJid) {
      originalMsg = messageCache.get(`${remoteJid}_${revokedId}`);
    }
