    ...
    try {
      ...
      this.sock = makeWASocket({
        ...
        getMessage: async (key) => {
          if (!key) return proto.Message.fromObject({});
        ...
        for (const msg of messages) {
          ...
          if (!safety.isBotEnabled()) {
            ...
            if (isCmd) {
              ...
              if ((cmdWord === 'bot' || cmdWord === 'viruz') && isOwner) {
      ...
      this.sock.ev.on('groups.upsert', (groups) => {
        try {
          for (const g of groups) if (g.id) antiDelete.cacheGroupMetadata(g.id, g);
        } catch (e) {}
      });
      this.sock.ev.on('groups.update', (updates) => {
        try {
          for (const g of updates) if (g.id) antiDelete.cacheGroupMetadata(g.id, g);
        } catch (e) {}
      });
