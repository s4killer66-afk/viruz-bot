  ...
  async start() {
    ...
    try {
      ...
      this.sock = makeWASocket({
        ...
        getMessage: async () => undefined  // Required for retries but we return nothing
      });
      ...
      this.sock.ev.on('messages.upsert', async (chatUpdate) => {
        try {
          // Only process new messages (not history sync / edits)
          if (chatUpdate.type !== 'notify') return;

          const msg = chatUpdate.messages[0];
          if (!msg) return;

          // ── Safety: skip junk messages ──
          ...
          // (anti-delete still works, but no commands fire)
          if (!safety.isBotEnabled()) {
            // Only allow .bot command from owner to re-enable
            const text = (
            ...
            if (isCmd) {
              ...
              if ((cmdWord === 'bot' || cmdWord === 'viruz') && safety.isOwner(msg.key.participant || msg.key.remoteJid)) {
          ...
          if (isGroup && !msg.key.fromMe) {
            ...
            // 1. Check for Sticker Spam (3 warn, 4 kick)
            if (msg.message.stickerMessage) {
              await moderator.handleStickerSpam(this.sock, from, sender, groupMetadata);
            }

            // 2. Check for Repeated Message Spam (4 warn, 5 kick)
