...
console.error = (...args) => {
  ...
  _origConsoleError.apply(console, args);
      ...
      this.sock.sendMessage = async (jid, content, options = {}) => {
        // 1. Natural Human Presence Simulation
        ...
        // 3. Dispatch message through socket
        const result = await rawSendMessage(jid, content, options);

        // 4. Return presence to paused
        if (typeof this.sock.sendPresenceUpdate === 'function') {
          try {
      ...
      this.sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
          ...
          if (isConflict) {
            ...
            console.log('⚠️ Another instance (e.g. KataBump or local) is running with this same WhatsApp session.');
